# Roadmap: MacroPulse v1.0 â Ship-Ready

## Overview

MacroPulse v0 shipped with all core capabilities: regime signal pipeline, REST API, dashboard, marketing site, and API docs. This milestone resolves every known critical issue before monetization â hardcoded secrets, race conditions, UI bugs, and UX gaps â leaving a clean, correct, and visually consistent product.
...
**Success Criteria** (what must be TRUE):
  ...
  2. A single alerting event fires exactly one alert â no duplicate notifications are observed
...
Plans:
- [ ] 01-env-example-audit-PLAN.md â Document all missing env vars in .env.example (SEC-01)
- [ ] 01-pipeline-fixes-PLAN.md â Remove duplicate regime alert call and fix data-lag threshold (SEC-02, BUG-01)
- [ ] 01-rate-limit-race-PLAN.md â Add per-IP asyncio.Lock to anonymous rate-limit counter (SEC-03)
- [x] 01-websocket-disconnect-PLAN.md â Snapshot _connections before broadcast iteration (BUG-02)
...
Plans:
- [ ] 02-01-PLAN.md â Create useCountdown hook, refactor Header.jsx (logo + hook), wire countdown into CommentaryCard (DASH-03, DASH-04)
- [ ] 02-02-PLAN.md â Fix calendar tier race, open WebhookGuide by default, fix RegimeCard date, verify all 7 DASH requirements (DASH-01, DASH-02, DASH-05, DASH-06, DASH-07)
...
Plans:
- [ ] 03-01-PLAN.md â Update hero H1 to "Stop predicting. Start allocating." and social meta tags (SITE-01)
- [ ] 03-02-PLAN.md â Change calendar default from 90 to 180 days; swap 6M button active state (SITE-04)
- [ ] 03-03-PLAN.md â Browser smoke-test all 4 SITE requirements (SITE-01, SITE-02, SITE-03, SITE-04)
...
**Success Criteria** (what must be TRUE):
  1. There is exactly one API docs page that contains all endpoint documentation â the detailed /api-docs content and the dashboard sidebar reference are unified
  2. The unified API docs page matches the dark visual style of macropulse.live (no light-mode or mismatched styling)
**Plans**: 2 plans

Plans:
- [ ] 04-01-sidebar-link-fix-PLAN.md â Fix "API Docs" sidebar link to point to macropulse.live/api-docs.html (DOCS-01)
- [ ] 04-02-api-docs-token-align-PLAN.md â Align api-docs.html CSS tokens to match index.html brand values (DOCS-02)
...
**Goal:** Fix internal pipeline reliability â silent data failures, missing HMM convergence guards, scattered magic number thresholds. No user-facing API changes.
**Requirements**: PIPE-01, PIPE-02, PIPE-03, PIPE-04, PIPE-05
**Depends on:** Phase 4
**Plans:** 4/4 plans complete

Plans:
- [ ] 05-00-PLAN.md â Install pytest, create test stubs and fixtures for all PIPE requirements (PIPE-01 through PIPE-05)
- [ ] 05-01-PLAN.md â Critical data halt (FRED/VIX), commodity column exclusion from PCA (PIPE-01, PIPE-02)
- [ ] 05-02-PLAN.md â HMM convergence guard, GARCH no-refit fix, narrow WebSocket except (PIPE-03, PIPE-04)
- [ ] 05-03-PLAN.md â Migrate 27 magic number thresholds to settings.py with env-var overrides (PIPE-05)

---
*Roadmap created: 2026-03-18 â v1.0 Ship-Ready milestone*
*Updated: 2026-03-18 â Phase 1 planned (4 plans)*
*Updated: 2026-03-18 â Phase 2 planned (2 plans)*
*Updated: 2026-03-18 â Phase 3 planned (3 plans)*
*Updated: 2026-03-18 â Phase 4 planned (2 plans)*
*Updated: 2026-03-19 â Phase 5 planned (4 plans)*

---

# Roadmap: MacroPulse v1.1 â Production Hardening

## Overview

v1.0 shipped a clean, visually consistent product with no known bugs. v1.1 closes the institutional deployment gap: secrets purged from git history, webhook bypass removed, auth endpoints hardened against brute-force, async DB driver replacing the sync bottleneck, Prometheus observability wired in, Paddle billing completed, GDPR erasure endpoint added, and automated test coverage established for all critical paths. Phases 6â12 deliver these capabilities in dependency order â security first, then DB, then observability, then billing and compliance, then tests that validate everything.
...
**Success Criteria** (what must be TRUE):
  ...
  2. Starting the API with `LS_WEBHOOK_SECRET` unset causes the Lemon Squeezy webhook endpoint to return 500 on every request â no event is processed
  ...
  4. The `model_artifacts` Docker volume is mounted with the `ro` flag in `docker-compose.yml` â a write attempt from the API container is rejected by the OS
  5. The application refuses to start when `ENV=production` and `CORS_ORIGINS` contains a wildcard `*`
**Plans**: 4 plans

Plans:
- [ ] 06-00-PLAN.md â Wave 0: test stubs for webhook and startup guard tests (SEC-20, SEC-21, SEC-22, SEC-42)
- [ ] 06-01-PLAN.md â Purge .env from git history, complete .env.example, create deployment guide (SEC-10, SEC-11, SEC-12, SEC-13)
- [ ] 06-02-PLAN.md â LS webhook fail-closed fix, startup guard for LS secret, Paddle replay verification (SEC-20, SEC-21, SEC-22)
- [ ] 06-03-PLAN.md â model_artifacts :ro volume, Nginx CSP header, CORS wildcard startup guard (SEC-40, SEC-41, SEC-42)
...
**Success Criteria** (what must be TRUE):
  ...
  3. Rate limit counters survive an API container restart â the 6th attempt after a restart still returns 429 if 5 attempts were made before restart
  4. OTP recovery endpoints return a backoff-indicating response after the 3rd failure within the window
**Plans**: 2 plans

Plans:
- [ ] 07-00-PLAN.md â Migration 009 (auth_rate_limits), check_and_record_attempt(), auth_rate_limit.py module, test stubs (SEC-33)
- [ ] 07-01-PLAN.md â Wire rate limits into register/verify/recover/recover_verify, implement 8 tests, update REQUIREMENTS.md (SEC-30, SEC-31, SEC-32, SEC-33)
...
**Success Criteria** (what must be TRUE):
  1. `database/connection.py` contains no import of `psycopg2` â `asyncpg` (or `psycopg3` async) is the only DB driver
  2. Every function in `database/queries.py` is declared `async def` â no synchronous DB calls exist anywhere in the API codebase
...
Plans:
- [x] 08-00-PLAN.md - asyncpg pool setup: database/connection.py + api/main.py lifespan (DB-10, DB-12)
- [x] 08-01-PLAN.md - Rewrite all 35+ query functions to asyncpg (DB-10, DB-11)
- [ ] 08-02-PLAN.md - Route handlers + middleware async wiring + test suite update (DB-10, DB-11, DB-13)
...
**Success Criteria** (what must be TRUE):
  ...
  4. Sending the same Paddle webhook event ID twice results in only one DB write â the second delivery is deduplicated via `webhook_idempotency`
...
**Success Criteria** (what must be TRUE):
  ...
  2. After account deletion, all `api_keys` for that user are deactivated â API calls with their keys return 401
...
**Success Criteria** (what must be TRUE):
  ...
  2. Paddle webhook tests cover `subscription.activated` and `subscription.cancelled` with both valid and invalid signatures â invalid signatures produce 401
...
**Execution Order:**
Phases execute in numeric order: 6 â 7 â 8 â 9 â 10 â 11 â 12

| Phase | Plans Complete | Status | Completed |
|-------|----------------|--------|-----------|
| 6. Secrets, Webhooks, and Infrastructure Hardening | 2/4 | In Progress|  |
| 7. Auth Endpoint Rate Limiting | 0/2 | Planned | - |
...
---
*v1.1 roadmap created: 2026-03-28 â Production Hardening milestone*
